...
class SigMCP(object):
    ...
    def colbert_rag_test(self, query, llm_set, prompt):
        ...
        Reproduces the ColBERT-RAG baseline from Table 2 of the Hi-RAG paper:
        flat retrieval using token-level MaxSim instead of pooled cosine.
        """
            ...
            for tool_i in tqdm(line.get('endpoints', [])):
                try:
                    ...
                    if not rag_type:
                        ...
                        if topk == 1:
                            response = self.colbert_rag_test(tool_i.get('query'), llm_set, prompt=prompt)
                        else:
                            response = self.colbert_rag_test_top3(tool_i.get('query'), llm_set, prompt=prompt)
                    elif rag_type == 'HIRAG':
                        if topk == 1:
